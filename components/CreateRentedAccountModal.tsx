...
const CreateRentedAccountModal: React.FC<CreateRentedAccountModalProps> = ({ isOpen, onClose, onSuccess }) => {
    ...
    const inputClasses = "w-full text-xl px-3 py-2 bg-slate-900/50 border-2 border-slate-600/50 rounded-md text-slate-100 placeholder-slate-400 focus:outline-none focus:border-cyan-400 transition-colors duration-300";
            ...
            <div className="bg-[#12122E]/90 w-full max-w-2xl border-2 border-cyan-400/30 shadow-[0_0_40px_rgba(0,255,255,0.2)]" style={{ clipPath: 'polygon(0 0, 100% 0, 100% calc(100% - 30px), calc(100% - 30px) 100%, 0 100%)' }}>
                <form onSubmit={handleSubmit}>
                    ...
                    <div className="p-8 space-y-6 max-h-[70vh] overflow-y-auto">
                        <input value={partnerName} onChange={e => setPartnerName(e.target.value)} placeholder="نام صاحب اصلی حساب (کرایه از)" required className={inputClasses} />
                        <input value={accountHolder} onChange={e => setAccountHolder(e.target.value)} placeholder="نام صاحب حساب در بانک (صرافی)" required className={inputClasses} />
                        <input value={bankName} onChange={e => setBankName(e.target.value)} placeholder="نام بانک (مثلا: ملت)" required className={inputClasses} />
                        <div className="grid grid-cols-2 gap-4">
                            <input value={accountNumber} onChange={e => setAccountNumber(e.target.value)} placeholder="شماره حساب" required className={inputClasses} />
                            <input value={cardNumber} onChange={e => setCardNumber(e.target.value)} placeholder="شماره کارت (اختیاری)" className={inputClasses} />
                        </div>
                        <div>
